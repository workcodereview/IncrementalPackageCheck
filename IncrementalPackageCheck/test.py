...
if __name__ == '__main__':
    ...
    #     print('root_info:', root_info)

    # 获取最新全包与更新包的aba_bundle信息
    analy = Analy_Plat()
    trunk_android_bundle, txpublish_android_bundle = analy.get_aba_bundle_dict()
    print('[Test]当前获取的主干包个数为:'+len(trunk_android_bundle)+'当前获取的分支包个数为:'+len(txpublish_android_bundle))
    ...
    total_singlenumber_message = {}  # 单号 < --- > 记录的主干和分支文件内容
    total_txpublish_meaage = {}  # 单号 < --- > 记录的分支文件内容
    count = 1 # 记录单号列表进度
    ...
    for single_number, root_info in single_number_assets.items():
        ...
        for index, value in result_total.items():
